...
def create_data_yaml(dataset_path, class_names):
    ...
    # FORCER exactement 5 classes EPI (ne pas détecter automatiquement)
    nc = 5  # len(class_names)  # Fixé à 5 classes
    
    # Noms de classes fixes
    names = ['helmet', 'glasses', 'person', 'vest', 'boots']  # class_names
...
def train_model(data_yaml, epochs=100, batch_size=16, img_size=640, weights='yolov5m.pt', retry_with_reduced_batch=True, session_name='epi_detection_v1'):
    """Lancer l'entraînement avec YOLOv5 OPTIMISÉ POUR PRÉCISION MAXIMALE"""
    yolov5_dir = Path('yolov5')
    ...
    if available_gb < batch_size * 1.0 and device == 'cpu':  # Ajusté pour précision (batch_size plus petit)
        print(f"⚠️  Mémoire disponible: {available_gb:.1f}GB, batch_size={batch_size} peut causer un crash")
        if retry_with_reduced_batch and batch_size > 8:
    ...
    print("\n" + "="*60)
    print("🚀 DÉMARRAGE DE L'ENTRAÎNEMENT YOLOv5 (OPTIMISÉ)")
    print("="*60)
    ...
    print(f"  - Optimisations: AdamW, Rect, Quad, CosLR, Workers=8, Patience=50, Multi-scale, Label smoothing, Freeze backbone")
    ...
    # Optimisations AGRESSIVES pour vitesse maximale
    # Déterminer cache et workers basés sur la RAM disponible
    cache_mode = 'ram' if available_gb > batch_size * 2 else 'disk'
    workers = 16 if available_gb > 8 else 12 if available_gb > 4 else 8
    ...
    cmd = [
        ...
        '--optimizer', 'AdamW',  # AdamW pour meilleure convergence
        '--rect',  # Rectangulaire dataloader
        '--quad',  # Quad split pour loading efficace
        '--cos-lr',  # Cosine annealing learning rate
        '--cache', cache_mode,  # Cache pour vitesse
        '--workers', str(workers),  # Workers optimisés
        '--patience', '50',  # Patience augmentée pour convergence
        '--label-smoothing', '0.1',  # Label smoothing pour régularisation
        '--multi-scale',  # Entraînement multi-échelle
        '--freeze', '10',  # Freeze les 10 premières couches pour fine-tuning
        '--hyp', str(yolov5_dir / 'data/hyps/hyp.scratch-med.yaml'),  # Hyperparamètres optimisés pour précision
    ]
    ...
    if best_model.exists():
        ...
        # Sauvegarder UNIQUEMENT best.pt
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=50, help='Nombre d\'epochs (default: 50 optimisé)')
    parser.add_argument('--batch-size', type=int, default=32, help='Taille du batch (default: 32 optimisé)')
    parser.add_argument('--img-size', type=int, default=416, help='Taille des images (default: 416 optimisé)')
    ...
    for train_idx in range(1, args.num_trainings + 1):
        ...
        if success:
            ...
            if best_model.exists():
                # Sauvegarder UNIQUEMENT best.pt au chemin global
